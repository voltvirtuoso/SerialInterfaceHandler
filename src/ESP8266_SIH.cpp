/**
 * @file ESP8266_SIH.cpp
 * @brief ESP8266 Serial Interface Handler - Core Implementation
 * 
 * This implementation file contains all method definitions for the ESP8266_SIH
 * system classes. It provides the complete functionality for WiFi management,
 * EEPROM storage operations, serial interface handling, menu system navigation,
...
// EEPROMStorage implementation
EEPROMStorage::EEPROMStorage() : _initialized(false) {}

void EEPROMStorage::loadHeader() {
    EEPROM.get(HEADER_ADDR, _header);
    if (_header.magic != EEPROM_MAGIC) {
        // Initialize new EEPROM structure
        _header.magic = EEPROM_MAGIC;
...
void EEPROMStorage::saveHeader() {
    EEPROM.put(HEADER_ADDR, _header);
    EEPROM.commit();
}

void EEPROMStorage::loadNetworks() {
    uint16_t addr = NETWORKS_ADDR;
    for (uint8_t i = 0; i < _header.networkCount && i < MAX_NETWORKS; i++) {
        EEPROM.get(addr, _networks[i]);
        addr += sizeof(NetworkCredential);
    }
}

void EEPROMStorage::saveNetworks() {
    uint16_t addr = NETWORKS_ADDR;
    for (uint8_t i = 0; i < _header.networkCount && i < MAX_NETWORKS; i++) {
        EEPROM.put(addr, _networks[i]);
        addr += sizeof(NetworkCredential);
    }
    EEPROM.commit();
}
...
int EEPROMStorage::findNetworkIndex(const String& ssid) const {
...
void EEPROMStorage::removeLeastSuccessfulNetwork() {
    ...
    for (uint8_t i = leastSuccessfulIndex; i < _header.networkCount - 1; i++) {
...
bool EEPROMStorage::saveNetwork(const String& ssid, const String& password) {
...
bool EEPROMStorage::getNetwork(const String& ssid, NetworkCredential& credential) {
    if (!_initialized) return false;
...
bool EEPROMStorage::removeNetwork(const String& ssid) {
    if (!_initialized) return false;
...
void EEPROMStorage::clearAllNetworks() {
...
std::vector<NetworkCredential> EEPROMStorage::getAllNetworks() const {
...
size_t EEPROMStorage::getNetworkCount() const {
    return _header.networkCount;
}

void EEPROMStorage::incrementConnectionAttempts(const String& ssid) {
    int index = findNetworkIndex(ssid);
    if (index >= 0) {
...
void EEPROMStorage::markConnectionSuccessful(const String& ssid) {
    int index = findNetworkIndex(ssid);
    if (index >= 0) {
        _networks[index].successfulConnections++;
        _networks[index].lastConnected = millis();
...
bool WiFiManager::scanNetworks(std::vector<WiFiScanResult>& results) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        result.isHidden = false; // ESP8266 doesn't provide this info
...
void MenuSystem::showSystemStatus() {
    ...
    _serial->printf("Chip Model: ESP8266\n");
    _serial->printf("EEPROM Size: %d bytes\n", EEPROM_SIZE);
...
// ESP8266_SIH Implementation
ESP8266_SIH::ESP8266_SIH(HardwareSerial* serial)
...
bool ESP8266_SIH::begin(uint32_t baudRate) {
    ...
    registerCommand("wifi", [this](const std::vector<String>& args) {
        if (args.size() > 0) {
            ...
            _serial->println("WiFi commands: scan, connect <ssid> <password>, status, auto");
        }
    ...
    _serial->println("\nESP8266 Serial Interface System initialized");
...
void ESP8266_SIH::autoReconnectWiFi() {
    auto status = _wifiManager.getStatus();
...
void ESP8266_SIH::registerCommand(const String& command, 
...
void ESP8266_SIH::setTimeout(unsigned long timeoutMs) {
    _timeout = timeoutMs;
    _menuSystem.setTimeout(timeoutMs);
}

bool ESP8266_SIH::isConnected() const {
    auto status = _wifiManager.getStatus();
    return status.connected;
}

String ESP8266_SIH::getConnectedSSID() const {
    auto status = _wifiManager.getStatus();
    return status.ssid;
}
